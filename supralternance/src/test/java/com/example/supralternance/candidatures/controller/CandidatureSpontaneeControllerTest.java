...
@WebMvcTest(controllers = CandidatureSpontaneeController.class)
public class CandidatureSpontaneeControllerTest {
    ...
    void testGetCandidatureSpontanee() throws Exception {
        ...
        Mockito.when(candidatureSpontaneeService.get(1)).thenReturn(candidature);

        mockMvc.perform(get("/candidatures/spontanees/1"))
    ...
    void testInsertCandidatureSpontanee() throws Exception {
        // Implémentez le test pour la méthode insert
        CandidatureSpontanee candidature = new CandidatureSpontanee();
        candidature.setIdCandidature(2);
        candidature.setNomCandidature("Stage : patissier");

        when(candidatureSpontaneeService.insert(Mockito.any(CandidatureSpontanee.class))).thenReturn(candidature);

        mockMvc.perform(post("/candidatures/spontanees").with(csrf())
